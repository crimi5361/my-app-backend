...
exports.affecterClasseEtGroupe = async (dbClient, { etudiantId, filiereNom, filiereSigle, niveauLibelle, cursus, typeFiliere, anneeAcademiqueId, filiereId, niveauId }) => {
  const nomClasse = `${filiereNom} ${filiereSigle} ${niveauLibelle}`;
  ...
  let classeResult = await dbClient.query(
    'SELECT id FROM classe WHERE filiere_id = $1 AND niveau_id = $2 AND annee_academique_id = $3',
    [filiereId, niveauId, anneeAcademiqueId]
  );
  let classeId;
  if (classeResult.rows.length > 0) {
    ...
    const newClasse = await dbClient.query(
      `INSERT INTO classe (nom, description, annee_academique_id, filiere_id, niveau_id) VALUES ($1, $2, $3, $4, $5) RETURNING id`,
      [nomClasse, `Classe pour ${descriptionClasse}`, anneeAcademiqueId, filiereId, niveauId]
    );
